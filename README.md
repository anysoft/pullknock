...
仓库内置 GitHub Actions CI、TestPyPI/PyPI Trusted Publishing、draft release 自动生成、release notes 检查、Dependabot、CodeQL、依赖安全扫描和 PR title 检查。更完整的版本策略见 [docs/11-release.md](./docs/11-release.md)。

## Release、TestPyPI 与 PyPI 发布流程

本项目使用 GitHub Actions 完成 CI、GitHub Release Draft、TestPyPI 测试发布和正式 PyPI 发布。推荐使用 PyPI Trusted Publishing / OIDC，不需要在 GitHub Secrets 中保存 PyPI Token。

### 发布前需要配置的 GitHub / PyPI 项
...
- `testpypi`：用于测试发布，可不设置人工审批。
- `pypi`：用于正式发布，建议设置 Required reviewers，避免误发布。

在 TestPyPI 配置 Trusted Publisher：

```text
Account settings -> Publishing -> Add a new pending publisher

Project name: pullknock
Owner: anysoft
Repository name: pullknock
Workflow filename: testpypi.yml
Environment name: testpypi
```

在正式 PyPI 配置 Trusted Publisher：
...
TestPyPI / PyPI 发布通过 GitHub OIDC Trusted Publishing 完成，不需要保存长期 API Token。

### 版本号维护规则

当前版本号需要手动维护，并且以下四处必须一致：

```text
pyproject.toml                 version = "0.1.0"
pullknock/__init__.py          __version__ = "0.1.0"
CHANGELOG.md                   ## v0.1.0
Git tag                        v0.1.0
```
...
python3 scripts/check_release_tag.py --tag v0.1.1 --require-tag
...
```bash
python3 -m compileall pullknock tests scripts
python3 -m unittest discover -s tests
python3 scripts/e2e_file_mode.py
python3 scripts/generate_config_schema_docs.py
git diff --exit-code docs/10-config-schema.md
```

### 标准发布流程

以发布 `0.1.1` 为例，先准备版本提交：

```bash
# 修改 pyproject.toml
# 修改 pullknock/__init__.py
# 修改 CHANGELOG.md

git add pyproject.toml pullknock/__init__.py CHANGELOG.md
...
```bash
git tag -a v0.1.1 -m "PullKnock v0.1.1"
git push origin v0.1.1
```

推送 `v*` tag 后会触发 `.github/workflows/release-draft.yml`。该 workflow 会检查 tag、版本号、CHANGELOG 是否一致，运行 release 检查，构建 sdist / wheel，生成 SBOM，创建或更新 GitHub Draft Release，并将构建产物附加到 draft release。

Release Draft 也可以手动触发：

```text
GitHub -> Actions -> Draft GitHub Release -> Run workflow
```
...
TestPyPI 用于正式发布前验证包能否正常安装。`.github/workflows/testpypi.yml` 支持两种触发方式：

- 推送 `v*` tag 自动触发。
- 在 GitHub Actions 页面手动运行，并输入 tag。
...
该 workflow 会 checkout 指定 tag，检查版本号和 tag，运行 release checks，构建包，检查 wheel/sdist，并发布到 TestPyPI。

发布完成后，新建干净环境测试：

```bash
python3 -m venv /tmp/pullknock-test
. /tmp/pullknock-test/bin/activate
...
如果 TestPyPI 安装正常，再继续正式发布。

### 正式 PyPI 发布流程

正式 PyPI 发布只通过 GitHub Release published 触发。进入：

```text
GitHub -> Releases
```

找到由 `release-draft.yml` 自动生成的 draft release，检查 tag、release notes、构建产物和 SBOM 是否正确。确认无误后点击：

```text
Publish release
```

发布 GitHub Release 后，会触发 `.github/workflows/publish.yml`，该 workflow 会自动发布到正式 PyPI。

发布完成后验证：

```bash
python3 -m venv /tmp/pullknock-pypi
. /tmp/pullknock-pypi/bin/activate

python3 -m pip install pullknock==0.1.1
...
| 流程 | Workflow | 触发方式 | 是否需要 tag |
| --- | --- | --- | ---: |
| 普通 CI | `ci.yml` | push / pull request | 否 |
| 安全扫描 | `security.yml` | push / pull request / schedule | 否 |
| Release Draft | `release-draft.yml` | push `v*` tag / 手动输入 tag | 是 |
| TestPyPI 发布 | `testpypi.yml` | push `v*` tag / 手动输入 tag | 是 |
| 正式 PyPI 发布 | `publish.yml` | GitHub Release published | 是 |
...
检查 `pyproject.toml`、`pullknock/__init__.py`、`CHANGELOG.md` 和 git tag，四处必须一致。

如果 `CHANGELOG.md` 缺少版本段，需要存在：

```markdown
## v0.1.1
```

并包含 release notes 模板：
...
如果 TestPyPI / PyPI 发布时报 OIDC / trusted publisher 相关错误，检查 PyPI/TestPyPI project name、GitHub owner、GitHub repository name、workflow filename 和 environment name 是否与 workflow 完全一致。

不要直接在 main 分支发布正式 PyPI。推荐流程是：

```text
push v* tag
  -> 生成 GitHub Draft Release
  -> TestPyPI 验证
