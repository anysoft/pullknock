#!/usr/bin/env python3
"""Generate Markdown documentation for PullKnock YAML configuration schemas."""

from __future__ import annotations

from pathlib import Path

OUTPUT = Path("docs/10-config-schema.md")
...
def main() -> int:
    ...
    for title, path, fields in SCHEMAS:
        chunks.append(f"## {title}\n\n默认路径：`{path}`\n\n| 字段 | 类型 | 说明 |\n| --- | --- | --- |\n")
        for field, field_type, description in fields:
            chunks.append(f"| `{field}` | `{field_type}` | {description} |\n")
        chunks.append("\n")
